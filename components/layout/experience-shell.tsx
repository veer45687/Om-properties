...
export function ExperienceShell({ children }: { children: React.ReactNode }) {
  ...
  return <>
    <motion.div className="fixed left-0 top-0 z-[90] h-1 origin-left bg-gradient-to-r from-amber-200 via-yellow-500 to-emerald-300" style={{ scaleX }} />
    ...
    <div className="pointer-events-none fixed z-[70] hidden h-72 w-72 -translate-x-1/2 -translate-y-1/2 rounded-full bg-amber-300/10 blur-3xl lg:block" style={{ left: pos.x, top: pos.y }} />
    ...
    <AnimatePresence>{splash && <motion.div className="fixed inset-0 z-[100] grid place-items-center bg-[#050504]" initial={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .7 }}><motion.div className="text-center" initial={{ scale: .82, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: .9, ease: 'easeOut' }}><motion.div className="mx-auto grid h-28 w-28 place-items-center rounded-[2rem] bg-gradient-to-br from-amber-100 via-yellow-500 to-amber-800 text-4xl font-black text-black shadow-[0_0_90px_rgba(215,168,63,.55)]" animate={{ rotate: [0, 0, -3, 3, 0], scale: [1, 1.08, 1] }} transition={{ duration: 2.2 }}>OM</motion.div><motion.h1 className="mt-6 font-[var(--font-poppins)] text-4xl font-extrabold gold-text" initial={{ y: 22, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ delay: .45 }}>Om Properties</motion.h1><motion.p className="mt-3 text-amber-100/70" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: .8 }}>Verified luxury real estate experiences</motion.p></motion.div></motion.div>}</AnimatePresence>
  </>;
}
