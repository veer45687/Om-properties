...
export function ContactForm({ propertyTitle }: { propertyTitle?: string }) {
  ...
  return (
    <form onSubmit={submit} className="glass dark-panel grid gap-4 rounded-[2rem] p-6" noValidate>
      ...
      <label><span className="text-sm text-white/70">Message</span><textarea aria-invalid={Boolean(errors.message)} rows={5} value={form.message} onChange={(e) => setForm({ ...form, message: e.target.value })} className="mt-2 w-full rounded-2xl border border-white/10 bg-black/25 px-4 py-4 outline-none focus:border-[#d8c08a]" placeholder="Tell us your preferred location, budget, and timeline" />{errors.message && <small className="text-[#d8c08a]">{errors.message}</small>}</label>
      <div className="flex flex-wrap gap-3"><Button disabled={submitting} size="lg" type="submit">{submitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Send className="mr-2 h-4 w-4" />}Submit Enquiry</Button><a className="inline-flex items-center rounded-full border border-[#9fbdb5]/40 px-6 py-3 font-semibold text-[#9fbdb5]" href={buildWhatsAppUrl(`${form.name || 'Customer'}: ${form.message || 'I want to enquire about Om Properties listings.'}`)}>WhatsApp Instead</a></div>
    </form>
  );
}
