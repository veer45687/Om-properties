...
export function PropertyCard({ property }: { property: Property }) {
  ...
  return (
    <article className="glass dark-panel group overflow-hidden rounded-[2rem] transition duration-300 hover:-translate-y-2 hover:border-amber-300/40 hover:shadow-[0_35px_100px_rgba(215,168,63,.12)]">
      <Link href={`/properties/${property.slug}`} className="relative block h-64 overflow-hidden">
        <Image sizes="(min-width: 1280px) 25vw, (min-width: 768px) 50vw, 100vw" fill src={property.images[0]} alt={property.title} className="object-cover transition duration-700 group-hover:scale-110" />
        ...
        <span className="absolute bottom-4 left-4 rounded-full bg-amber-300 px-3 py-1 text-xs font-black text-black">{property.status}</span>
      </Link>
      <div className="p-5">
        <div className="flex items-center justify-between gap-3">
          <p className="flex items-center gap-2 text-sm text-white/60"><MapPin className="h-4 w-4" />{property.location}</p>
          <div className="flex gap-2">
            <button aria-label="Save property" onClick={() => setLiked((value) => !value)} className={`rounded-full border p-2 transition ${liked ? 'border-rose-300 bg-rose-400 text-white' : 'border-white/10 bg-white/5 text-white/60 hover:text-rose-300'}`}><Heart className="h-4 w-4" fill={liked ? 'currentColor' : 'none'} /></button>
            <button aria-label="Compare property" onClick={() => setCompare((value) => !value)} className={`rounded-full border p-2 transition ${compare ? 'border-amber-300 bg-amber-300 text-black' : 'border-white/10 bg-white/5 text-white/60 hover:text-amber-300'}`}><GitCompare className="h-4 w-4" /></button>
          </div>
        </div>
        ...
        <button className="mt-4 flex items-center gap-2 text-xs text-white/45 transition hover:text-amber-200"><Share2 className="h-3 w-3" />Share / print ready details</button>
      </div>
    </article>
  );
}
